...
class UploadHandler {
    ...
    public function getName(){
        if (__request::get('__filename'))
            return __request::get('__filename');
    ...
    public function handleUpload($uploadDirectory, $name = null){
        ...
        // Save a chunk
        $totalParts = __request::get('__totalparts') ? (int)__request::get('__totalparts') : 1;

        $uuid = __request::get('__uuid');
        if (!$uuid) {
        ...
        if ($totalParts > 1){
            ...
            $partIndex = (int)__request::get('__partindex');
            ...
            if (__config::get('APPLICATION_STATE') == 'development') {
                $success = rename($_FILES[$this->inputName]['tmp_name'], $target);
            } else {
            ...
            if ($target){
                $this->uploadName = $uuid.DIRECTORY_SEPARATOR.basename($target);

                if (!is_dir(dirname($target))){
                    mkdir(dirname($target));
                }
                if (__config::get('force_upload_with_rename')) {
